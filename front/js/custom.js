...
//**************************** CUSTOM JS SECTION ****************************************
  ...
  let check_url_main = check_url[check_url.length - 1];
  if (check_url_main == 'checkout') {
    state();
  }

    function state() {
      let set_state_url = $('#change_country option:selected').attr('data-href');
      $.get(set_state_url , function (res) {
        $('#set_state').html(res);
      });
    }
      ...
      $.ajax({
        ...
        success: function (data) {
          if ((data.errors)) {
            ...
            $this.find('.alert-danger ul').html('');
            for (var error in data.errors) {
              $('.mloginform .alert-danger p').html(data.errors[error]);
            }
          } else {
            $this.find('.alert-info').hide();
            $this.find('.alert-danger').hide();
...
function getAmount()
{
  ...
  return total;
    ...
    $(document).on('click', '.product-color .color-list .box', function () {
      ...
      $('.hidden-add-to-cart').attr('rel', colors);
        var parent = $(this).parent();
...
prices = $(".product-attr:checked").map(function() {
   return $(this).data('price');
    ...
    $(document).on("click", "#qaddcrt" , function(){
      ...
      window.location = mainurl+"/addtonumcart?id="+pid+"&qty="+qty+"&size="+sizes+"&color="+colors+"&size_qty="+size_qty+"&size_price="+size_price+"&size_key="+size_key+"&keys="+keys+"&values="+values+"&prices="+prices;
...
$(document).on('click', '.hidden-add-to-cart', function(e){
  ...
  let colors_check = $(this).attr('rel');
  let size_check = $(this).attr('rel1');
  
  console.log(size_check,colors_check);
    
        if(typeof size_check !== "undefined" || (typeof colors_check !== "undefined")){
